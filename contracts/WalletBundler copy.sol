...
contract WalletBundler_COPY {
    ...
    function _transferFromToken(address token, address from, address to, uint256 amount) private {
        assembly {
            ...
            // CRITICAL FIX: Use returndatacopy to prevent memory corruption
            let success := call(gas(), token, 0, ptr, 0x64, 0, 0)
            if iszero(success) { revert(0, 0) }

            // Copy return data if exists (some tokens return bool, some return nothing)
            if gt(returndatasize(), 0) {
                let outPtr := add(ptr, 0x80)
                returndatacopy(outPtr, 0, returndatasize())
                let returnValue := mload(outPtr)
        ...
        for (; i < stepsLength;) {
            ...
            if (tokenIn != address(0)) {
                ...
                assembly {
                    ...
                    if success {
                        if gt(returndatasize(), 0) {
                            // Copy return data to safe location
                            let outPtr := add(ptr, 0x80)
                            returndatacopy(outPtr, 0, 0x60)
                ...
                if (permit2Allowance < APPROVAL_THRESHOLD) {
                    ...
                    _approvePermit2(tokenIn, target);  // Permit2 → Spender (uint160 max, does decrement)
                }
            }
     ...
     * @dev Handles USDT and other tokens that require approve(0) before new approval
     */
    function _approve(address token, address spender) private {
        assembly {
            let ptr := mload(0x40)

            // First, try to approve 0 (required by USDT and some other tokens)
            // This is safe to call for all tokens - standard ERC20s ignore it
            mstore(ptr, 0x095ea7b300000000000000000000000000000000000000000000000000000000)
            mstore(add(ptr, 0x04), spender)
            mstore(add(ptr, 0x24), 0) // approve 0 first

            // Call approve(spender, 0) - ignore return value
            pop(call(gas(), token, 0, ptr, 0x44, 0, 0))

            // Now approve max amount
            mstore(ptr, 0x095ea7b300000000000000000000000000000000000000000000000000000000)
            mstore(add(ptr, 0x04), spender)
    ...
    /**
     * @dev Approve Permit2 spending (safer version using abi.encodeWithSelector)
    ...
    function _approvePermit2(address token, address spender) private {
        ...
        (bool success, ) = PERMIT2.call(approveData);
        if (!success) revert CallFailed();
    }
