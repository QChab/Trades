...
/**
 * Helper functions to handle both string and object token formats
 */
...
export async function useMixedUniswapBalancer({
  ...
  try {
    ...
    const allTokens = [
      ...
      ...intermediates.map((intermediate) => intermediate.address)
    ];
    ...
    if (useBalancer || useUniswap) {
      ...
      if (useUniswap && uniswapPools) {
        ...
        console.log('📦 Fetching Uniswap pools with intermediates...');
        // Fetch pools for direct pair AND all intermediate routes
        const specificPools = await uniswap.findPossiblePools(tokenInObject, tokenOutObject, intermediates).catch(err => {
          console.log(`   No pools for ${tokenInObject.symbol}→${tokenOutObject.symbol}: ${err.message}`);
          return [];
        });
        allUniswapPools = specificPools;
        console.log(`   Fetched ${allUniswapPools.length} Uniswap pools (including intermediate routes)`);
      }
    ...
    if (useUniswap && useBalancer) {
      ...
      [uniswapRoutes, balancerRoutes] = await Promise.all([
        ...
        discoverBalancerPaths(tokenInObject, tokenOutObject, amountInBN, provider, allBalancerPools, intermediates)
      ]);
    } else if (useUniswap) {
      // Only Uniswap
      ...
      // Only Balancer
      balancerRoutes = await discoverBalancerPaths(tokenInObject, tokenOutObject, amountInBN, provider, allBalancerPools, intermediates);
    }
 ...
 * @param {Array} intermediates - Array of intermediate token addresses for multi-hop routing
 */
 ...
 * @param {Array} intermediates - Array of intermediate token addresses for multi-hop routing
 */
async function discoverBalancerPaths(tokenInObject, tokenOutObject, amountIn, provider, prefetchedPools = [], intermediates = []) {
  try {
    ...
    // Use passed intermediates for multi-hop routing (normalize to lowercase)
    // Balancer uses WETH, so convert ETH addresses to WETH
    const normalizedIntermediates = intermediates.filter(token => token.address !== ETH_ADDRESS).map(inter => inter.address.toLowerCase());


    const relevantTokens = [tokenInAddr, tokenOutAddr, ...normalizedIntermediates];
 ...
 */
/**
 * Discover cross-DEX paths combining Uniswap and Balancer
 * @param {Array} intermediates - Array of intermediate token objects {address, symbol, decimals}
 */
async function discoverCrossDEXPaths(tokenIn, tokenOut, amountIn, provider, prefetchedUniswapPools = [], prefetchedBalancerPools = [], intermediates = []) {
  const paths = [];
  ...
  console.log(`\n🔀 Trying ${intermediates.length} intermediates for ${tokenIn.symbol} → ${tokenOut.symbol}:`);
  console.log(`   Intermediates: ${intermediates.map(t => t.symbol).join(', ')}`);
  ...
  for (const intermediate of intermediates) {
    ...
    try {
      ...
      if (uniswapLeg1 && uniswapLeg1.length > 0 && uniswapLeg1[0].outputAmount?.gt(0)) {
        leg1Options.push({ leg: uniswapLeg1[0], protocol: 'uniswap' });
      }
      if (balancerLeg1 && balancerLeg1.length > 0 && balancerLeg1[0].outputAmount?.gt(0)) {
        leg1Options.push({ leg: balancerLeg1[0], protocol: 'balancer' });
      }
      ...
      for (const leg1Option of leg1Options) {
        ...
        for (const secondIntermediate of intermediates) {
          if (secondIntermediate.address === intermediate.address ||
              ...
              secondIntermediate.address === tokenOut.address) {
            continue;
          }
          ...
          try {
            ...
            const intermediateForUniswap = intermediate.isETH
              ? { ...intermediate, address: ethers.constants.AddressZero, symbol: 'ETH' }
              : intermediate;
            const secondIntermediateForUniswap = secondIntermediate.isETH
              ? { ...secondIntermediate, address: ethers.constants.AddressZero, symbol: 'ETH' }
              : secondIntermediate;

            // Leg 2: Try BOTH Uniswap and Balancer
            const [leg2Uniswap, leg2Balancer] = await Promise.all([
              discoverUniswapPaths(intermediateForUniswap, secondIntermediateForUniswap, leg1Output, prefetchedUniswapPools),
              discoverBalancerPaths(intermediate, secondIntermediate, leg1Output, provider, prefetchedBalancerPools, intermediates)
            ]);

            const leg2Options = [];
            if (leg2Uniswap && leg2Uniswap.length > 0) {
              const bestLeg2Uni = selectBestVariant(leg2Uniswap);
              if (bestLeg2Uni && bestLeg2Uni.outputAmount?.gt(0)) {
                leg2Options.push({ leg: bestLeg2Uni, protocol: 'uniswap' });
              }
            }
            if (leg2Balancer && leg2Balancer.length > 0) {
              const bestLeg2Bal = selectBestVariant(leg2Balancer);
              if (bestLeg2Bal && bestLeg2Bal.outputAmount?.gt(0)) {
                leg2Options.push({ leg: bestLeg2Bal, protocol: 'balancer' });
              }
            }
            ...
            for (const leg2Option of leg2Options) {
              ...
              for (const thirdIntermediate of intermediates) {
                if (thirdIntermediate.address === secondIntermediate.address ||
                    ...
                    thirdIntermediate.address === tokenIn.address ||
                    thirdIntermediate.address === tokenOut.address) {
                  continue;
                }
                ...
                try {
                  // Handle ETH/WETH conversion for Uniswap
                  const secondIntermediateForUniswapLeg3 = secondIntermediate.isETH
                    ? { ...secondIntermediate, address: ethers.constants.AddressZero, symbol: 'ETH' }
                    : secondIntermediate;
                  const thirdIntermediateForUniswap = thirdIntermediate.isETH
                    ? { ...thirdIntermediate, address: ethers.constants.AddressZero, symbol: 'ETH' }
                    : thirdIntermediate;

                  // Leg 3: Try BOTH Uniswap and Balancer
                  const [leg3Uniswap, leg3Balancer] = await Promise.all([
                    discoverUniswapPaths(secondIntermediateForUniswapLeg3, thirdIntermediateForUniswap, leg2Output, prefetchedUniswapPools),
                    discoverBalancerPaths(secondIntermediate, thirdIntermediate, leg2Output, provider, prefetchedBalancerPools, intermediates)
                  ]);

                  const leg3Options = [];
                  if (leg3Uniswap && leg3Uniswap.length > 0) {
                    const bestLeg3Uni = selectBestVariant(leg3Uniswap);
                    if (bestLeg3Uni && bestLeg3Uni.outputAmount?.gt(0)) {
                      leg3Options.push({ leg: bestLeg3Uni, protocol: 'uniswap' });
                    }
                  }
                  if (leg3Balancer && leg3Balancer.length > 0) {
                    const bestLeg3Bal = selectBestVariant(leg3Balancer);
                    if (bestLeg3Bal && bestLeg3Bal.outputAmount?.gt(0)) {
                      leg3Options.push({ leg: bestLeg3Bal, protocol: 'balancer' });
                    }
                  }
                  ...
                  for (const leg3Option of leg3Options) {
                    ...
                    const leg4Options = [];
                    if (leg4Uniswap && leg4Uniswap.length > 0) {
                      const bestLeg4Uni = selectBestVariant(leg4Uniswap);
                      if (bestLeg4Uni && bestLeg4Uni.outputAmount?.gt(0)) {
                        leg4Options.push({ leg: bestLeg4Uni, protocol: 'uniswap' });
                      }
                    }
                    if (leg4Balancer && leg4Balancer.length > 0) {
                      const bestLeg4Bal = selectBestVariant(leg4Balancer);
                      if (bestLeg4Bal && bestLeg4Bal.outputAmount?.gt(0)) {
                        leg4Options.push({ leg: bestLeg4Bal, protocol: 'balancer' });
                      }
                    }
                    ...
                    for (const leg4Option of leg4Options) {
                      ...
                      for (const legOption of [leg1Option, leg2Option, leg3Option, leg4Option]) {
                      ...
                      if (!hasReusedPool) {
                        ...
                        const protocols = [leg1Option.protocol, leg2Option.protocol, leg3Option.protocol, leg4Option.protocol];
...
function buildTokenPairGroups(routes) {
  ...
  routes.forEach((route, routeIndex) => {
    ...
    if (route.legs && route.legs.length > 0) {
      ...
      if (poolInfo) {
        const tokenPairKey = `${poolInfo.inputToken?.symbol || poolInfo.inputToken}->${poolInfo.outputToken?.symbol || poolInfo.outputToken}`;
...
function buildPoolExecutionStructure(routes, splitPercentages) {
  ...
  routes.forEach((route, routeIndex) => {
    ...
    route.legs.forEach((leg, legIndex) => {
      ...
      if (poolKey) {
        ...
        const outputTokenSymbol = getTokenSymbol(poolInfo.outputToken);
        if (!tokenProducers.has(outputTokenSymbol)) {
          tokenProducers.set(outputTokenSymbol, []);
        }
        if (!tokenProducers.get(outputTokenSymbol).includes(poolKey)) {
          tokenProducers.get(outputTokenSymbol).push(poolKey);
        }
      }
    });
  });
  ...
  while (remainingPools.size > 0) {
    ...
    for (const poolKey of remainingPools) {
      ...
      const inputProducers = tokenProducers.get(inputTokenSymbol) || [];
...
function initializeFromRoutes(routes, groupsWithLevels, totalAmount) {
  ...
  for (let routeIdx = 0; routeIdx < routes.length; routeIdx++) {
    ...
    // Handle both multi-hop routes (with legs) and direct routes (with paths)
    const routeLegs = route.legs || route.paths || [];

    if (routeLegs.length > 0) {
      // console.log(`         Route ${routeIdx + 1} (${routeDesc}): ${routeLegs.length} legs`);
...
async function optimizeInterGroupSplit(
  ...
  for (let i = 0; i < numGroups; i++) {
    if (removedIndices.has(i)) {
      const outputSymbol = getTokenSymbol(competingGroups[i].outputToken);
      removedOutputTokens.add(outputSymbol);
    }
  }

  // For each removed output token, check if there are other groups producing it
  for (const token of removedOutputTokens) {
    // CRITICAL: Check ALL groups (not just competingGroups) for other producers
    // E.g., if SEV→ETH is removed at level 1, ONE→ETH at level 0 still produces ETH
    const hasOtherProducers = allGroupsWithLevels.some(g =>
      getTokenSymbol(g.outputToken) === token && !removedGroupKeys.has(g.tokenPairKey)
    );

    console.log(`      🔍 Token "${token}" removed. Other producers exist? ${hasOtherProducers}`);
    ...
    if (!hasOtherProducers) {
      ...
      const downstreamGroups = allGroupsWithLevels.filter(g =>
        ...
        getTokenSymbol(g.inputToken) === token
      );

      console.log(`      🔗 Cascading removal to ${downstreamGroups.length} downstream group(s) that consume ${token}`);
      for (const downstream of downstreamGroups) {
        console.log(`         • ${downstream.tokenPairKey}`);
  ...
  for (let i = 0; i < numGroups; i++) {
    ...
    const groupTokenIn = { symbol: inputSymbol, decimals: tokenDecimalsLookup[inputSymbol] || 18, address: tokenIn.address };
    const groupTokenOut = { symbol: outputSymbol, decimals: tokenDecimalsLookup[outputSymbol] || 18, address: tokenOut.address };
...
async function evaluateInterGroupSplit(
  ...
  for (let level = currentLevel + 1; level <= maxLevel; level++) {
    ...
    // CRITICAL: Identify competing groups (same input token) at this level
    const inputTokenGroups = new Map(); // inputToken -> [groups]
    for (const group of groupsAtLevel) {
      const inputToken = getTokenSymbol(group.inputToken);
      if (!inputTokenGroups.has(inputToken)) {
        inputTokenGroups.set(inputToken, []);
      }
      inputTokenGroups.get(inputToken).push(group);
    }

    // Process each input token (and its competing groups)
    for (const [inputToken, groups] of inputTokenGroups) {
      // Calculate total input available for this token
      let totalInputForToken = BigNumber.from(0);

      // Sum outputs from current evaluation
      for (const [tpKey, output] of groupOutputs) {
        const prevGroup = allGroupsWithLevels.find(g => g.tokenPairKey === tpKey);
        if (prevGroup && getTokenSymbol(prevGroup.outputToken) === inputToken) {
          totalInputForToken = totalInputForToken.add(output);
        }
      }

      // Also add from previousOptimizations if not yet calculated
      for (const [tpKey, opt] of previousOptimizations) {
        if (!groupOutputs.has(tpKey)) {
          const prevGroup = allGroupsWithLevels.find(g => g.tokenPairKey === tpKey);
          if (prevGroup && getTokenSymbol(prevGroup.outputToken) === inputToken) {
            totalInputForToken = totalInputForToken.add(opt.totalOutput);
          }
        }
      }

      if (totalInputForToken.eq(0)) continue;

      if (groups.length === 1) {
        // Sequential group - gets 100% of input
        ...
        for (const group of groups) {
          const prevOpt = previousOptimizations.get(group.tokenPairKey);
          let groupInputPct = 1.0 / groups.length; // Default: equal split

          if (prevOpt && prevOpt.inputPercentage !== undefined) {
            groupInputPct = prevOpt.inputPercentage;
          }

          splits.push(`${(groupInputPct * 100).toFixed(1)}%`);

          const groupInput = totalInputForToken.mul(Math.floor(groupInputPct * 1000000)).div(1000000);
...
function assignLevelsToGroups(tokenPairGroups, initialInputToken) {
  ...
  for (const group of groupsWithLevels) {
    ...
    if (!tokenProducers.has(outputSymbol)) {
      tokenProducers.set(outputSymbol, []);
    }
    tokenProducers.get(outputSymbol).push(group);
  }
  ...
  const initialInputSymbol = getTokenSymbol(initialInputToken);
  for (const group of groupsWithLevels) {
    if (getTokenSymbol(group.inputToken) === initialInputSymbol) {
      group.level = 0;
...
async function optimizeSplitSimple(routes, totalAmount, tokenIn, tokenOut) {
  ...
  let bestGlobalOutput = BigNumber.from(0);
  let bestGlobalIteration = -1;
  let bestGroupOptimizations = new Map();
  let bestGroupInputAmounts = new Map();
  let bestGroupInitialAllocations = new Map(groupInitialAllocations);

  console.log(`\n🌐 Starting global optimization across ${maxLevel + 1} levels...`);

  for (globalIteration = 0; globalIteration < maxGlobalIterations; globalIteration++) {
    console.log(`\n📍 Global Iteration ${globalIteration + 1}/${maxGlobalIterations}`);

    // CRITICAL: Reset removed groups each iteration - don't permanently exclude groups
    const allRemovedGroupKeys = new Set(); // Track groups removed WITHIN THIS ITERATION ONLY

    // Process each level sequentially (dependencies flow from lower to higher levels)
    for (let level = 0; level <= maxLevel; level++) {
      // Filter out groups that were removed in THIS iteration
      ...
      if (groupsAtLevel.length === 0 && level > 0) {
        // Debug: why are there no groups at this level?
        const allGroupsAtLevel = groupsWithLevels.filter(g => g.level === level);
        if (allGroupsAtLevel.length > 0) {
          console.log(`   ⚠️  All ${allGroupsAtLevel.length} groups at level ${level} were removed:`);
          for (const g of allGroupsAtLevel) {
            console.log(`      ${g.tokenPairKey} - removed: ${allRemovedGroupKeys.has(g.tokenPairKey)}`);
          }
        }
      }
      ...
      if (competingGroups.length === 1) {
        ...
        if (level === 0) {
          ...
          for (const [tpKey, opt] of groupOptimizations) {
            ...
            if (prevGroup && getTokenSymbol(prevGroup.outputToken) === getTokenSymbol(group.inputToken)) {
        ...
        if (level === 0) {
          ...
          const inputTokenSymbol = getTokenSymbol(competingGroups[0].inputToken);
          for (const [tpKey, opt] of groupOptimizations) {
            const prevGroup = groupsWithLevels.find(g => g.tokenPairKey === tpKey);
            if (prevGroup && getTokenSymbol(prevGroup.outputToken) === inputTokenSymbol) {
        ...
        // In global mode: 30 iterations per level for thorough optimization
    ...
    // Calculate current global output for convergence check
    let currentGlobalOutput = BigNumber.from(0);
    for (const group of groupsWithLevels) {
      const opt = groupOptimizations.get(group.tokenPairKey);
      if (opt && group.outputToken === tokenOut.symbol) {
        currentGlobalOutput = currentGlobalOutput.add(opt.totalOutput);
      }
    }

    // Check if this iteration is better than the best so far
    if (currentGlobalOutput.gt(bestGlobalOutput)) {
      console.log(`\n🎯 New best solution found in iteration ${globalIteration + 1}! ${ethers.utils.formatUnits(currentGlobalOutput, tokenOut.decimals)} ${tokenOut.symbol}`);
    ...
    if (globalIteration > 0) {
      const improvement = currentGlobalOutput.sub(previousGlobalOutput);
      const improvementPct = previousGlobalOutput.isZero()
        ? 100
        : improvement.mul(10000).div(previousGlobalOutput).toNumber() / 100; // Basis points to %

      console.log(`\n📊 Global iteration ${globalIteration + 1} output: ${ethers.utils.formatUnits(currentGlobalOutput, tokenOut.decimals)} ${tokenOut.symbol} (${improvementPct > 0 ? '+' : ''}${improvementPct.toFixed(2)}%)`);

      // Warn if output decreased
      if (improvement.lt(0)) {
        console.log(`⚠️  Warning: Output decreased in global iteration ${globalIteration + 1}`);
      }

      // DISABLED: Don't exit early, run all 20 iterations to find true optimum
      // if (improvement.gte(0) && improvement.lte(previousGlobalOutput.div(10000))) {
      //   console.log(`✅ Global optimization converged after ${globalIteration + 1} iterations (improvement < 0.01%)`);
      //   break;
      // }
    } else {
      console.log(`\n📊 Global iteration 1 output: ${ethers.utils.formatUnits(currentGlobalOutput, tokenOut.decimals)} ${tokenOut.symbol}`);
    }

    previousGlobalOutput = currentGlobalOutput;

    // CRITICAL: Update groupInitialAllocations for next global iteration
    // Use the optimized inputPercentage from this iteration as starting point for next iteration
    console.log(`\n🔄 Updating initial allocations for next iteration:`);
    for (const [tokenPairKey, opt] of groupOptimizations) {
  ...
  console.log(`\n✅ Using best solution from iteration ${bestGlobalIteration}/${maxGlobalIterations}: ${ethers.utils.formatUnits(bestGlobalOutput, tokenOut.decimals)} ${tokenOut.symbol}`);
  groupOptimizations.clear();
  for (const [key, value] of bestGroupOptimizations) {
    groupOptimizations.set(key, value);
  }
  groupInputAmounts.clear();
  for (const [key, value] of bestGroupInputAmounts) {
    groupInputAmounts.set(key, value);
  }
  ...
  for (const group of groupsWithLevels) {
    ...
    // Sum final outputs
    if (group.outputToken === tokenOut.symbol) {
      const groupOutput = ethers.utils.formatUnits(opt.totalOutput, tokenOut.decimals);
      console.log(`      ✓ Adding ${group.tokenPairKey} output: ${groupOutput} ${tokenOut.symbol}`);
      totalOptimizedOutput = totalOptimizedOutput.add(opt.totalOutput);
    } else {
      console.log(`      → Intermediate output: ${group.outputToken} (feeds Level ${group.level + 1})`);
    }
  }
...
function buildPoolExecutionStructureFromGroups(tokenPairGroups, groupOptimizations) {
  ...
  for (const group of tokenPairGroups) {
    ...
    for (const pool of group.pools) {
      ...
      // Get token symbols early for reuse in multiple checks
      const inputTokenSymbol = getTokenSymbol(pool.inputToken);
  ...
  const levels = Array.from(levelMap.entries())
    ...
    .map(([levelNum, pools]) => {
      ...
      consolidatedPools.forEach(pool => {
        const inputTokenSymbol = getTokenSymbol(pool.inputToken);
        if (!inputTokenGroups.has(inputTokenSymbol)) {
          inputTokenGroups.set(inputTokenSymbol, []);
        }
        inputTokenGroups.get(inputTokenSymbol).push(pool);
      });

      // CRITICAL: Consolidate competing output tokens with tiny allocations
      // E.g., if SEV→EIG has 0.008% and SEV→ETH has 99.992%, consolidate SEV→EIG into SEV→ETH
      const MIN_ALLOCATION = 0.0002; // 0.02%
      const finalConsolidatedPools = [];

      inputTokenGroups.forEach((poolGroup, inputToken) => {
        // Group by output token to find competing destinations
        const outputTokenGroups = new Map(); // outputToken -> [pools]
        poolGroup.forEach(pool => {
          const outputToken = getTokenSymbol(pool.outputToken);
          if (!outputTokenGroups.has(outputToken)) {
            outputTokenGroups.set(outputToken, []);
          }
          outputTokenGroups.get(outputToken).push(pool);
        });

        // Calculate total percentage for each output token
        const outputTotals = new Map();
        outputTokenGroups.forEach((pools, outputToken) => {
          const totalPct = pools.reduce((sum, p) => sum + (p.percentage || 0), 0);
          outputTotals.set(outputToken, totalPct);
        });

        // Find tiny output tokens (< 0.02%) and the largest output token
        const tinyOutputs = [];
        let largestOutput = null;
        let largestPct = 0;

        outputTotals.forEach((pct, outputToken) => {
          if (pct < MIN_ALLOCATION && pct > 0) {
            tinyOutputs.push(outputToken);
            console.log(`      🔄 Consolidating tiny output ${inputToken}→${outputToken} (${(pct * 100).toFixed(4)}%)`);
          } else if (pct > largestPct) {
            largestPct = pct;
            largestOutput = outputToken;
          }
        });

        // Consolidate tiny outputs into largest output
        if (tinyOutputs.length > 0 && largestOutput) {
          const largestPools = outputTokenGroups.get(largestOutput);
          const largestPool = largestPools[largestPools.length - 1]; // Use last pool (usually the main one)

          tinyOutputs.forEach(tinyOutput => {
            const tinyPools = outputTokenGroups.get(tinyOutput);
            tinyPools.forEach(tinyPool => {
              largestPool.inputAmount = (largestPool.inputAmount || ethers.BigNumber.from(0)).add(tinyPool.inputAmount || ethers.BigNumber.from(0));
              largestPool.expectedOutput = (largestPool.expectedOutput || ethers.BigNumber.from(0)).add(tinyPool.expectedOutput || ethers.BigNumber.from(0));
              largestPool.percentage = (largestPool.percentage || 0) + (tinyPool.percentage || 0);
            });
            console.log(`      ✓ Consolidated ${inputToken}→${tinyOutput} into ${inputToken}→${largestOutput} (new: ${(largestPool.percentage * 100).toFixed(2)}%)`);
            outputTokenGroups.delete(tinyOutput); // Remove tiny output
          });
        }

        // Add remaining pools to final result
        outputTokenGroups.forEach((pools) => {
          finalConsolidatedPools.push(...pools);
        });
      });
      ...
      // Re-group by input token for sorting
      const inputTokenGroupsFinal = new Map();
      finalConsolidatedPools.forEach(pool => {
        const inputTokenSymbol = getTokenSymbol(pool.inputToken);
        if (!inputTokenGroupsFinal.has(inputTokenSymbol)) {
          inputTokenGroupsFinal.set(inputTokenSymbol, []);
        }
        inputTokenGroupsFinal.get(inputTokenSymbol).push(pool);
      });

      inputTokenGroupsFinal.forEach((poolGroup, inputToken) => {
...
async function selectBestRoute(routes, amountIn, tokenIn, tokenOut) {
  ...
  // Log all routes being considered
  console.log(`\n🔍 selectBestRoute: Comparing ${routes.length} routes:`);
  routes.forEach((route, i) => {
    const output = route.totalOutput ? ethers.utils.formatUnits(route.totalOutput, tokenOut.decimals) : 'N/A';
    console.log(`   ${i + 1}. ${route.type || 'unknown'}: ${output} ${tokenOut.symbol}`);
  });
  ...
  // Log selected route
  const selectedOutput = selected.totalOutput ? ethers.utils.formatUnits(selected.totalOutput, tokenOut.decimals) : 'N/A';
  console.log(`\n✅ Selected: ${selected.type || 'unknown'} with ${selectedOutput} ${tokenOut.symbol} (arbitrageRisk: ${selected.arbitrageRisk.toFixed(3)})`);
