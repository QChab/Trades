...
// Lazy load V4 SDK modules - will be initialized on first use
let Pool, Trade;
let v4sdkLoaded = false;

async function ensureV4SDKLoaded() {
  if (!v4sdkLoaded) {
    try {
      ...
      Trade = v4sdk.Trade;
      v4sdkLoaded = true;
      console.log('✅ Loaded V4 SDK from fixed ESM module');
    } catch (error) {
      console.warn('Failed to load ESM module, falling back to regular import:', error);
      // Fallback to regular import
      const v4sdk = await import('@uniswap/v4-sdk');
      Pool = v4sdk.Pool;
      Trade = v4sdk.Trade;
      v4sdkLoaded = true;
    }
...
export function useUniswapV4() {
  ...
  async function findPossiblePools(tokenInObject, tokenOutObject) {
    ...
    let candidatePools = Array.from(unique.values()).filter((pool) => (
      pool.hooks === '0x0000000000000000000000000000000000000000' && pool.liquidity > 100000 && pool.totalValueLockedUSD && Number(pool.totalValueLockedUSD) >= 100
    ))
    ...
    for (const pool of candidatePools) {
      ...
      // Ensure V4 SDK is loaded before creating Pool
      await ensureV4SDKLoaded();
  ...
  async function selectBestPath(tokenInObject, tokenOutObject, pools, rawIn, simpleMultiRoute = true) {
    ...
    try {
      ...
      // In test file, default to simple multi-route mode (maxHops: 1)
      const maxHops = simpleMultiRoute ? 1 : 2;
      trades = await Trade.bestTradeExactIn(pools, amountIn, tokenB, { maxHops, maxNumResults: 8 });
  ...
  async function findAndSelectBestPath(tokenInObject, tokenOutObject, amountIn, simpleMultiRoute = true) {
